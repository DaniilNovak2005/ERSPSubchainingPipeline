#!/usr/bin/env bash
# run_sailor_bzip2.sh — Run the full SAILOR pipeline on bzip2 only.
#
# Usage:
#   bash run_sailor_bzip2.sh
#
# Requires:
...
if [ -f "$COMMIT_CACHE" ]; then
    ...
    echo "[Setup] Using cached commit ${COMMIT} (${BZIP2_TAG})."
else
    echo "[Setup] Cloning bzip2 ${BZIP2_TAG} to discover commit hash..."
    TEMP_CLONE="dataset/_bzip2_tmp"
    mkdir -p dataset/
    git clone --depth 1 --branch "$BZIP2_TAG" "$BZIP2_REPO" "$TEMP_CLONE"
    ...
    DATASET_PATH="dataset/${COMMIT}/bzip2_${COMMIT}_vul"
...
if [ ! -d "${DATASET_PATH}/.git" ]; then
    echo "[Setup] Dataset missing — cloning bzip2 ${BZIP2_TAG}..."
    mkdir -p "$(dirname "$DATASET_PATH")"
    git clone --depth 1 --branch "$BZIP2_TAG" "$BZIP2_REPO" "$DATASET_PATH"
    echo "[Setup] Clone done."
    echo ""
else
    echo "[Setup] bzip2 dataset already present, skipping clone."
    echo ""
fi
