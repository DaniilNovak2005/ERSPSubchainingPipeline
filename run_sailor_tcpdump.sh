#!/usr/bin/env bash
# run_sailor_tcpdump.sh — Run the full SAILOR pipeline on tcpdump 4.9.2.
#
# Usage:
#   bash run_sailor_tcpdump.sh [--force-specs 12,34,56]
#
#   --force-specs <ids>   Comma-separated spec ID numbers to force re-run
#                         even if they already have results.
#
# Requires:
...
COMMIT_CACHE="dataset/.tcpdump_commit"

if [ -f "$COMMIT_CACHE" ]; then
    COMMIT_FULL=$(cat "$COMMIT_CACHE")
    COMMIT="${COMMIT_FULL:0:8}"
    echo "[Setup] Using cached commit ${COMMIT} (${TCPDUMP_TAG})."
else
    echo "[Setup] Cloning tcpdump ${TCPDUMP_TAG} to discover commit hash..."
    TEMP_CLONE="dataset/_tcpdump_tmp"
    mkdir -p dataset/
    git clone --depth 1 --branch "$TCPDUMP_TAG" "$TCPDUMP_REPO" "$TEMP_CLONE"
    ...
    DATASET_PATH="dataset/${COMMIT}/tcpdump_${COMMIT}_vul"
    mkdir -p "$(dirname "$DATASET_PATH")"
    mv "$TEMP_CLONE" "$DATASET_PATH"
fi

# --- Project identifiers ---
PROJECT="tcpdump_${COMMIT}_vul"
...
if [ ! -d "${DATASET_PATH}/.git" ]; then
    echo "[Setup] Dataset missing — cloning tcpdump ${TCPDUMP_TAG}..."
    mkdir -p "$(dirname "$DATASET_PATH")"
    git clone --depth 1 --branch "$TCPDUMP_TAG" "$TCPDUMP_REPO" "$DATASET_PATH"
    echo "[Setup] Clone done."
    echo ""
else
    echo "[Setup] tcpdump dataset already present, skipping clone."
    echo ""
fi
...
if [ -f "${FINDINGS_JSON}" ] && [ -f "${PROJECT_BC}" ] && [ "${SPEC_COUNT}" -gt 0 ]; then
    ...
    SPEC_COUNT=0
    [ -d "${SPECS_DIR}" ] && SPEC_COUNT=$(find "${SPECS_DIR}" -maxdepth 1 -name "*.json" | wc -l)
    echo "[Phase 1] Done. Generated ${SPEC_COUNT} specs."
fi
echo ""
